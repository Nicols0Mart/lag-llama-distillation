...
logging.basicConfig(filename=f'training_fold_{str(int(time.time()))}_debug.log', level=logging.INFO, 
...
def create_train_dataset_without_last_k_timesteps(
    ...
    train_data = ListDataset(train_data, freq=freq, one_dim_target=False,one_dim_target_static=True)
    return train_data
...
def dataset_factory_pems(
    ...
    def to_deepar_format(dataframe, time_feature, index=None, dataset_name="PEMS04"):
        ...
        if windows:
            ...
            data = [{
                        ...
                        FieldName.TARGET: time_feature[i],
                        FieldName.FEAT_DYNAMIC_REAL: time_feature[i],
                        FieldName.FEAT_STATIC_REAL: lpls[[i] + knn_map[i]] if not JUST_K else lpls[i],
                        FieldName.ITEM_ID: i,
                        "data_id": data_id
                    } 
                    for i, c in enumerate(dataframe.columns)]
        return ListDataset(data, freq=freq, one_dim_target=False, one_dim_target_static=True)
    ...
    for i, series in enumerate(raw_train_dataset.train):
        ...
        if val_start_date is not None:
            ...
            train_end_index = len(series["target"]) - num_val_windows if series["target"].ndim == 1 else len(series["target"][0]) - num_val_windows
    ...
    val_tmp = ListDataset(val_tmp, freq=freq, one_dim_target=False, one_dim_target_static=True)
...
def test_dataset_factory_pems(
    ...
    feat_time_04 = [ddd[:, :train_num_short] for ddd in aggregated_04]
    feat_time_04[0].shape
    def to_deepar_format(dataframe, time_feature, index=None, dataset_name="PEMS04", knn_map=None):
        freq = "5min"
        start_index = datetime.datetime.strptime(date, "%Y-%m-%d")
        # day_data, week_data, _ = time_add(dataframe, **dataset_params[dataset_name])
        # if len(dataframe.shape) == 2:
        ...
        if windows:
            ...
            data = [{
                        FieldName.START: start_index if (index is None and windows is None) else start_index + datetime.timedelta(minutes=index * 5),
                        FieldName.TARGET: np.vstack([dataframe[c].values[np.newaxis,:], time_feature[i]]),
                        FieldName.FEAT_DYNAMIC_REAL: time_feature[i],
                        FieldName.FEAT_STATIC_REAL: lpls[[i] + knn_map[i]] if not JUST_K else lpls[i],
                        FieldName.ITEM_ID: i,
                        "data_id": data_id
                    ...
                    for i, c in enumerate(dataframe.columns)]
        return ListDataset(data, freq=freq, one_dim_target=False, one_dim_target_static=True)
    test_data = to_deepar_format(train_data_04, feat_time_04, dataset_name=name, knn_map=knn_map)
...
for k_index, train_dataset_names in enumerate(all_train_dataset_names):
    ...
    #  val_data = CombinedDataset(val_datasets, weights=None)
     ...
     estimator.trainer_kwargs["max_epochs"] = 1*(k_index+2)
...
for data_id, name in enumerate(fine_tune_datasets):
     ...
     logger.info(
     "Dataset:" + str(name) + 
     "Total train points:"+ str(total_train_points)+
     "Total val points:"+ str(total_val_points)
     )
...
def calc_metrics(ground_truth, prediction, horizon=5):
    ...
    for i in range(1, len(prediction)):
        ...
        try:
          ...
          reals = torch.cat([reals, torch.from_numpy(real.values).unsqueeze(-1)], dim=1)
